  ...
  describe('ValkeyCache', () => {
    ...
    describe('get', () => {
      it('should retrieve and parse cached data', async () => {
        ...
        vi.mocked(valkeyClient.get).mockResolvedValue(JSON.stringify(mockData));
      ...
      it('should handle JSON parsing errors gracefully', async () => {
        vi.mocked(valkeyClient.get).mockResolvedValue('invalid json');

        await expect(cache.get('bad-key')).rejects.toThrow();
      });
    ...
    describe('set', () => {
      it('should serialize and cache data with TTL', async () => {
        ...
        vi.mocked(valkeyClient.setex).mockResolvedValue('OK');

        const result = await cache.set('key2', mockData);

        expect(result).toBe(true);
        expect(valkeyClient.setex).toHaveBeenCalledWith(
          'test:key2',
          3600,
      ...
      it('should return false on cache write failure', async () => {
        ...
        vi.mocked(valkeyClient.setex).mockResolvedValue(null);
      ...
      it('should handle serialization errors', async () => {
        ...
        await expect(cache.set('circular-key', circularData)).rejects.toThrow();
      });
    });

    describe('delete', () => {
      it('should delete cached data', async () => {
        vi.mocked(valkeyClient.del).mockResolvedValue(1);

        const result = await cache.delete('key5');

        expect(result).toBe(true);
        expect(valkeyClient.del).toHaveBeenCalledWith('test:key5');
      });

      it('should return false when key does not exist', async () => {
        vi.mocked(valkeyClient.del).mockResolvedValue(0);
      ...
      it('should handle deletion errors', async () => {
        vi.mocked(valkeyClient.del).mockRejectedValue(new Error('Delete error'));

        await expect(cache.delete('error-key')).rejects.toThrow('Delete error');
      });
    });

    describe('exists', () => {
      it('should return true when key exists', async () => {
        vi.mocked(valkeyClient.exists).mockResolvedValue(1);
      ...
      it('should return false when key does not exist', async () => {
        vi.mocked(valkeyClient.exists).mockResolvedValue(0);
